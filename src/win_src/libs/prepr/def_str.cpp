  ...
  for( i = 0; i >= 0; i++)
   {
    ...
    buf->inp_p = buf->ilex[buf->inp_p];
    if( buf->inp_p < 0 )   /* end of chain: lsplex[-1] is outside the buffer */
     {
      mac1[i] = '\0';
      return( BUF_EOF );
      };
  ...
  for( i = 0; i >= 0; i++)
   {
    ...
    buf->inp_p = buf->ilex[buf->inp_p];
    if( buf->inp_p < 0 ) { put_error_messege( "Small buffer" ); return( BUF_EOF ); }
    ch = buf->lsplex[buf->inp_p];
    switch( ch )
     {
      case '(' : jb++;
  ...
  for( i = 0; i < n; i++)
   {
    ...
    p = buf->ilex[p];
    if( p < 0 ) return( BUF_EOF );   /* end of chain: lsplex[-1] is outside the buffer */
